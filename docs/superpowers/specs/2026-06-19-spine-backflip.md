...
The following items depend on the live mjlab configuration inside `mjlab-dev` or on the outcome of the reference retargeting step. They are unresolved without a Spark probe and are marked for resolution in Task 12 of the implementation plan before the training run begins.

1. **Reference feasibility and duration.** The backflip reference in `smpl_backflip.pkl` may differ in duration, frame count, or kinematic feasibility from the cartwheel reference. The `pkl_to_csv --duration` bug that produced a double-cartwheel could recur if not checked. **⚠ verify on Spark:** run `play_motion_npz.py` on the converted `backflip.npz`; confirm the clip is a single backflip; note the total duration in seconds.

2. **Exact termination-threshold flag names in mjlab.** The training command above uses `--env.terminations.anchor-pos.threshold` and `--env.terminations.ee-body-pos.threshold`. The actual YAML key names in the tracking task config may differ (e.g. `anchor_pos_threshold`, `ee_pos`, or similar). **⚠ verify on Spark:** inspect the cartwheel run's `params/env.yaml` (`logs/rsl_rl/g1_tracking/2026-04-19_20-54-47_cartwheel-iterC-single/params/env.yaml`) for the exact termination key paths. The flags passed to the train command must match these paths exactly or the override silently fails.

3. **Whether 0.5 m is the right threshold for a backflip.** The cartwheel succeeded at 0.5 m, but a backflip involves a more vertical rotation and a different end-effector trajectory than a cartwheel. It is possible that 0.5 m is still too tight (the policy needs more tolerance during the fully-airborne phase) or slightly too loose (the policy exploits the tolerance to find a cheap local minimum that isn't a real backflip). **⚠ verify on Spark:** after iterA, check whether the visual review shows genuine rotation or a flop; adjust threshold for iterB if needed and document the reason.

4. **`record_policy.py` flag names for threshold and disable-terminations.** The spec uses `--termination-threshold` and `--disable-terminations`. These flags were added to `record_policy.py` during the cartwheel project (noted in `cartwheel-journey.md`). **⚠ verify on Spark:** run `python scripts/record_policy.py --help` and confirm these exact flag names; adjust the commands above if the names differ.
