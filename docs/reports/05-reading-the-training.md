...
Once the robot can walk at all, episode length saturates. By iteration 900, episodes are already averaging 962 steps out of a 1,000-step cap. There is almost no room left to go higher. But reward keeps climbing, because a robot that now survives its whole episode has 1,000 timesteps in which to earn velocity-tracking points, smoothness points, and upright-posture points it could not collect before. Episode length hits its ceiling and plateaus; reward keeps rising as the policy gets better at *what it does with the time it has*.
