...
Once the robot can walk at all, episode length approaches its ceiling. By iteration 900, episodes are already averaging 962 steps out of a 1,000-step cap — there is almost no room left to go higher, and the small dip to 925 at iteration 1,400 is within normal run-to-run training noise — Chapter 07 will show why two training runs never match exactly. But reward keeps climbing, because a robot that now survives its whole episode has 1,000 timesteps in which to earn velocity-tracking points, smoothness points, and upright-posture points it could not collect before. Episode length hovers near its ceiling; reward keeps rising as the policy gets better at *what it does with the time it has*.
