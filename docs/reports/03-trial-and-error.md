...
- **`direction in θ that made the taken action more likely`** — the gradient proper: of all the ways we could tweak the 200,000 weights, this is the combination that would make the policy *more inclined* to choose the action it actually chose. The neural network computes this direction by backpropagation through the same network that produced the action — cheap and automatic with modern autodiff.
